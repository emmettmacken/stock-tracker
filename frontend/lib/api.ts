...
import { BASE, fetchWithAuth } from "./auth";

export { BASE };

export async function fetchQuote(ticker: string): Promise<QuoteData> {
  const res = await fetchWithAuth(`${BASE}/api/quote/${ticker}`);
  if (!res.ok) {
...
export async function fetchBacktest(ticker: string): Promise<BacktestData> {
  const res = await fetchWithAuth(`${BASE}/api/backtest/${ticker}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: res.statusText }));
...
export async function fetchFactors(ticker: string): Promise<FactorScoreData> {
  const res = await fetchWithAuth(`${BASE}/api/factors/${ticker}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: res.statusText }));
...
export async function fetchCompany(ticker: string): Promise<CompanyInfo> {
  const res = await fetchWithAuth(`${BASE}/api/company/${ticker}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: res.statusText }));
...
export async function fetchSentiment(ticker: string): Promise<SentimentData> {
  const res = await fetchWithAuth(`${BASE}/api/sentiment/${ticker}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: res.statusText }));
...
export async function fetchInsider(ticker: string): Promise<InsiderData> {
  const res = await fetchWithAuth(`${BASE}/api/insider/${ticker}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: res.statusText }));
...
export async function fetchShortInterest(ticker: string): Promise<ShortInterestData> {
  const res = await fetchWithAuth(`${BASE}/api/shortinterest/${ticker}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: res.statusText }));
...
export async function fetchPortfolioSizing(req: {
  ...
  const res = await fetchWithAuth(`${BASE}/api/portfolio/sizing`, {
    method: "POST",
...
export async function fetchPortfolioBacktest(req: {
  ...
  const res = await fetchWithAuth(`${BASE}/api/portfolio/backtest`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(req),
  });
...
export async function fetchWatchlistDB(): Promise<WatchlistTicker[]> {
  const res = await fetchWithAuth(`${BASE}/api/watchlist`);
  if (!res.ok) throw new Error("Failed to fetch watchlist");
  return res.json();
}

export async function addTickerDB(ticker: string): Promise<void> {
  const res = await fetchWithAuth(`${BASE}/api/watchlist/${ticker}`, { method: "POST" });
...
export async function fetchWatchlistSnapshot(): Promise<SnapshotData[]> {
  const res = await fetchWithAuth(`${BASE}/api/watchlist/snapshot`);
...
export async function fetchUpcomingEarnings(days = 30): Promise<UpcomingEarnings> {
  const res = await fetchWithAuth(`${BASE}/api/earnings/upcoming?days=${days}`);
...
export async function refreshTicker(ticker: string): Promise<SnapshotData> {
  const res = await fetchWithAuth(`${BASE}/api/watchlist/${ticker}/refresh`, { method: "POST" });
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: res.statusText }));
...
export async function removeTickerDB(ticker: string): Promise<void> {
  const res = await fetchWithAuth(`${BASE}/api/watchlist/${ticker}`, { method: "DELETE" });
...
export async function fetchPortfolioHistory(period: string): Promise<PortfolioHistory> {
  const res = await fetchWithAuth(`${BASE}/api/portfolio/history?period=${period}`);
...
export async function fetchLiveEquity(): Promise<{ equity: number; timestamp: string }> {
  const res = await fetchWithAuth(`${BASE}/api/portfolio/live-equity`);
...
export async function fetchEntrySignals(): Promise<EntrySignals> {
  const res = await fetchWithAuth(`${BASE}/api/portfolio/positions/entry-signals`);
...
export async function closePosition(
  ...
  const res = await fetchWithAuth(`${BASE}/api/portfolio/positions/close`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function fetchLockedPositions(): Promise<string[]> {
  const res = await fetchWithAuth(`${BASE}/api/portfolio/positions/locked`);
...
export async function lockPosition(
  ...
  const res = await fetchWithAuth(`${BASE}/api/portfolio/positions/lock`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function unlockPosition(
  ...
  const res = await fetchWithAuth(`${BASE}/api/portfolio/positions/unlock`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ticker }),
  });
...
export async function triggerSignalJob(): Promise<{ status: string; message: string }> {
  const res = await fetchWithAuth(`${BASE}/api/paper/run-now`, { method: "POST" });
  if (!res.ok) throw new Error("Failed to trigger signal job");
  return res.json();
}

export async function fetchAnalytics(): Promise<AnalyticsData> {
  const res = await fetchWithAuth(`${BASE}/api/analytics`);
  if (!res.ok) throw new Error("Failed to fetch analytics");
  return res.json();
}

export async function fetchFactorContribution(): Promise<FactorContributionData> {
  const res = await fetchWithAuth(`${BASE}/api/analytics/factor-contribution`);
...
export async function fetchGateRejections(): Promise<GateRejectionsData> {
  const res = await fetchWithAuth(`${BASE}/api/analytics/gate-rejections`);
  if (!res.ok) throw new Error("Failed to fetch gate rejections");
  return res.json();
}

export async function fetchDrawdown(): Promise<DrawdownData> {
  const res = await fetchWithAuth(`${BASE}/api/analytics/drawdown`);
...
export async function fetchPriceHistory(
  ...
  const query = max
    ...
    : `days=${days}`;
  const res = await fetchWithAuth(`${BASE}/api/price-history/${ticker}?${query}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: res.statusText }));
...
export async function fetchTradingSettings(): Promise<TradingSettings> {
  const res = await fetchWithAuth(`${BASE}/api/settings/trading`);
